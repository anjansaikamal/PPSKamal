...
#include<conio.h>
#include<string.h>
struct golden_airline
{
int passport[6];
char name[15];
int seat_num;
char email[15];
struct golden_airline *following;
}
*begin, *stream;
struct golden_airline *dummy;
void main()
{
...
do
{
printf("Welcome To golden Airlines Online Booking Hub\n");
...
{
// first user
begin = stream = (struct golden_airline*)malloc(sizeof(struct golden_airline));
details();
...
stream->following = (struct golden_airline *)malloc(sizeof(struct golden_airline));
stream = stream->following;
details();
stream->following = NULL;
...
void savefile()
{
FILE *fpointer = fopen("golden Records", "w");
if (!fpointer)
{
...
printf("Details Have Been Saved To A File (golden Records)");
fclose(fpointer);
}
void display()
{
